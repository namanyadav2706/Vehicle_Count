...
def displayPersonCount(frame, person_count):
	cv2.putText(
		frame, #Image
		'Detected Persons: ' + str(person_count), #Label
		(20, 50), #Position
		cv2.FONT_HERSHEY_SIMPLEX, #Font
		0.8, #Size
		(0, 0xFF, 0), #Color
...
def count_persons(idxs, boxes, classIDs, person_count, previous_frame_detections, frame):
	current_detections = {}
	# ensure at least one detection exists
	if len(idxs) > 0:
		# loop over the indices we are keeping
...
# frame dimensions
videoStream = cv2.VideoCapture(0) #inputVideoPath
...
num_frames, vehicle_count , person_count = 0, 0, 0        #*************************
...
while True:
	...
	person_count, current_detections2 = count_persons(idxs, boxes, classIDs, person_count, previous_frame_detections2, frame)
	...
	#***************
	previous_frame_detections2.pop(0)
